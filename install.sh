...
# Используем default сеть от Supabase
networks:
  default:
...
services:
  traefik:
    ...
    command:
      ...
      - "--providers.docker.network=${PROJECT_NAME}_default"
  ...
  n8n:
    ...
    environment:
      ...
      - N8N_PROTOCOL=http
      - DB_TYPE=postgresdb
      - DB_POSTGRESDB_HOST=supabase-db  # Используем Supabase DB!
      - DB_POSTGRESDB_PORT=5432
      - DB_POSTGRESDB_DATABASE=n8n
      - DB_POSTGRESDB_USER=n8n
      - DB_POSTGRESDB_PASSWORD=${N8N_DB_PASSWORD}
      - EXECUTIONS_MODE=${N8N_EXEC_MODE:-regular}
      - QUEUE_BULL_REDIS_HOST=redis
      - QUEUE_BULL_REDIS_PASSWORD=${REDIS_PASSWORD}
    depends_on:
      - redis
      - supabase-db  # Зависит от Supabase DB
    healthcheck:
...
# Для FULL режима добавляем worker

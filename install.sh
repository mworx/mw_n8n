#!/bin/bash
# НЕ ИСПОЛЬЗУЙТЕ "set -e" В ЭТОМ БЛОКЕ!

# Проверяем, запущен ли скрипт через pipe (неинтерактивно)
if [ ! -t 0 ]; then
    TEMP_SCRIPT=$(mktemp)
    cat > "$TEMP_SCRIPT"
    chmod +x "$TEMP_SCRIPT"
...
check_system_requirements() {
    ...
    local ram_gb=$((total_ram / 980))
    if [[ $total_ram -ge 3860 ]]; then
        echo -e "${GREEN}${CHECK_MARK} ${ram_gb}GB${NC}"
    else
        echo -e "${YELLOW}⚠ ${ram_gb}GB (рекомендуется ≥4GB+)${NC}"
    fi
